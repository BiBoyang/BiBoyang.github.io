...
tags: [iOS, Objective-C, property, runtime]
---
...
但是这里有个问题：**对象的内存布局在编译期已经被固定了**。当你访问这个变量的时候，编译器就会将其替换为指针偏移量。这个偏移量是硬编码的，表示变量距离对象内存起始地址有多远。

但是假如又加了一个变量，就要重新编译。

这种问题大体有两种解决方案：

1. 把实例变量当成一种带偏移描述的信息交给类对象管理，让运行时在需要的时候再去取对应偏移；
2. 不直接访问实例变量，而是通过属性方法，也就是 `getter/setter` 来进行处理。

# 1. 原理

从使用层面看，**@property = 实例变量 + getter 方法 + setter 方法**。

当使用 `self.xx` 的时候，如果是设置值，那么就是在调用 `setter` 方法，如果是获取值，那就是在调用 `getter` 方法。这也是 `getter` 方法中为何不能用 `self.xx` 的原因。
...
`self.name` 实际上就是执行了属性 `name` 的 `getter` 方法，`getter` 方法中又调用了`self.name`，会一直递归调用，直到程序崩溃.
...
而 `objc_property` 对应到底层的一个结构体，里面最核心的是 `name` 和 `attributes`，定义如下：
...
- `property_t` 里的 `attributes`，本质上是一段编码后的属性描述字符串；
- `objc_property_attribute_t` 则是 runtime API 在操作属性时使用的键值对形式。
...
我们使用 **property_getAttributes** 方法，就能拿到包括类型、原子性、内存语义以及实例变量名等信息。后面我们可以看到相关代码。
...
`_ivar_t` 结构体表示每一个实例变量，记录了**偏移值、名称、类型、对齐方式和大小**，用于描述每一个实例变量。
...
`_method_list_t` 结构体表示类的实例方法列表，记录了每一个**实例方法的大小、实例方法个数以及具体的实例方法描述**，每加入一个属性则会在 `_method_list_t` 中增加 `setter` 与 `getter` 方法的描述。
...
这里的锁，是 `PropertyLocks` 这一组按地址分条管理的锁，底层类型是 `StripedMap`。

而最后，`atomic` 修饰的对象，会通过 `objc_autoreleaseReturnValue` 这条路径交给 autorelease 语义去处理。
...
* atomic : 是否是原子操作
* copy : 是否按 `copy` 语义处理
* mutableCopy : 是否按 `mutableCopy` 语义处理
...
1. 只有在 ARC 下这个方法才会出现（试验代码的情况下）
2. 只有当前类拥有实例变量时（不论是不是用 property）这个方法才会出现，且父类的实例变量不会导致子类拥有这个方法
