...
在做 Agent 工程质量时候，我有一个观点：trace 是一等公民，测试断言、评测数据都从它而来。这个观点有些受我之前做 iOS 性能 trace 的影响,但是也是自己实际经历过几个 agent 开发以及深入调研后的思考的结果.

这条主线在近期 deepseek harness 发布之后，被我总结成为一个开源项目 [dsh-eval-harness](https://github.com/BiBoyang/dsh-eval-harness),这是一个给 dsh 插件跑回归的门禁工具。它最近在回归测试 DeepSeek Harness 时抓到一个上游自己都没发现的并发崩溃：三个进程同时启动，两个在 270 毫秒内 ENOENT 崩掉。

这篇文章我准备按照以下路线：
1. trace 怎么设计才对（地基）
2. 拿 trace 做 eval 要防什么（方法）
3. 评测 harness 的关键决策与实现管线（工程）
4. 它抓到上游 bug 的全过程（战果）。

# 一、trace 要在项目第一天就做

这个认知，是三个项目用三种姿势分别验证出来的。一个从第一天就做对了，一个晚了两个月补票，一个把它推到了我没预想到的远度。为了保密三个项目匿名，但每个坑都是真的。

## 做对的：让「界面状态」成为可断言对象

最早做的是一个 Swift 终端 UI 库，渲染 AI 流式输出用的。终端程序有个经典难题：界面状态很难复现,断言脆弱的字节流等于没测，渲染逻辑一重构测试就全红。

经过我和深入思考和朋友反复套,我想到了一个解法:第二轮开发开始时内建了一个 VirtualTerminal：一个能在内存里解释 ANSI 转义序列的虚拟终端（字符网格 + 光标 + CSI 序列解析）。测试不再断言「输出了什么字节」，而是断言「最终屏幕状态」。配合事件溯源式的渲染模型——UI 状态完全由事件流驱动，不绑定任何业务模型——渲染的每个关键决策都变成可断言的诊断事件：什么时候全量重绘（带原因）、什么时候走增量更新、快速路径追加了多少行。
...
## 番外：三个一线开源 Agent 是怎么做 trace + eval 的

写完正文，我把三个一线 Coding Agent 的实现翻了一遍：Kimi Code CLI（MoonshotAI/kimi-cli，Python 开源）、Codex CLI（openai/codex，Rust 开源）、Claude Code（TypeScript，官方文档加公开工程博客）。动机很朴素：正文那些结论都是从个人项目里长出来的，我想知道它们在大厂的工程体系里是被证实还是被证伪。

大部分被证实了，读到的时候我是松了口气的——这些坑不是只有我一个人踩。差异集中在三处，后面逐个说。

先说印证的部分，正文 §一 的几条标准，三家各自用代码投过票。

事件总线是标配。三家的架构概念上同构：core 产出带类型标签的事件流，UI 只是订阅者。Kimi 的 Wire 协议是 24 种 Event 加 4 种 Request 的广播通道，TUI、IDE、Web、`kimi vis` 可视化器全吃同一份 wire.jsonl；Codex 把这条总线做成了正式产品——JSON-RPC app-server，协议类型能直接导出 TypeScript 和 JSON Schema；Claude Code 干脆不暴露内部总线，把可观测性整个标准化成 OpenTelemetry 三信号，企业用户接进自己现成的观测栈。形态不同，结论相同：core 和 UI 之间必须隔一条结构化事件流。

格式带版本号也没人不做。Kimi 的 wire.jsonl 首行就是 `{"type":"metadata","protocol_version":"1.10"}`；Codex 用 serde alias 兜住改名后的旧事件，协议形状变更必须重新生成 schema fixture，相当于给协议上了 snapshot 锁。正文 §五 的报告 schemaVersion 和 loader 兼容逻辑，是同一套思路。

打分纪律上更没有分歧。三家对外成绩（SWE-bench、Terminal-Bench）全部是 test-based，跑真实仓库的测试判对错。LLM-judge 只用于 rubric 评审，而且 Anthropic 的《Demystifying evals for AI agents》里那套纪律——judge 上岗前用人工标注校准、grader 防 bypass（他们真实遇到过 agent 翻历史 trial 的 git 记录作弊）、pass@k 和 pass^k 分清、必须人工读 transcript——和正文 §三 §四 几乎逐条对应。

然后是三处差异，每处都值得单说。

**一、假模型替身不是用不用的问题，是在哪一层用的问题。** 正文 §四 说「读真实 trace，不用 LLM 替身」，这在端到端层成立。但 Codex 的 `core/tests/suite/` 里有 129 个集成测试，全部用 wiremock 起假模型服务器、拼 SSE 事件流、驱动真实实例跑完整 turn，再对出站的 API 请求体做结构化断言；Kimi 的协议 e2e 测试（W-01 到 W-42）也用 scripted echo 假 provider。

这不矛盾，是分层。三家的实践合起来是个四层金字塔：
...
假模型在 L3 是对的：它测的是「框架把模型回答转化成了什么动作」，要的就是确定性，烧真 token 反而引入噪音。真实链路在 L4 也是对的：它验证整条链路在真实模型行为下能不能活。dsh-eval-harness 站在 L4。正文批评的替身，错在拿 L3 的手段顶 L4 的岗——「替身测通过的系统上线照样崩」依然成立，因为 L3 本来就不对上线负责。

**二、路径断言和结果断言，防的是两种不同的假。** Anthropic 主张「评 outcome 而非路径」，正文 §二 主张「断言路径而不是结果」，放一起看像打架。其实 Anthropic 防的是 grader 误伤合法解法——agent 走了你没预想的路径但结果正确，不能判 fail；正文防的是能力零覆盖的假通过——想测 read_image，它用 bash 的 file 兜底答对。合起来才是完整纪律：断言「能力必须被真实调用」这个覆盖前提，但不断言「必须按我设想的方式解题」；结果对不对，交给 outcome grader。

**三、被测系统该暴露什么钩子。** 要给自家 agent 建 eval，被测系统得提供四样东西，三家各贡献了一部分样本：

1. **可注入的 run 标识**。让 harness 能把本次评测的 run ID 注入 agent（比如通过环境变量打进遥测事件），事后才能把遥测数据和 eval 结果 join 起来按用例切片。
2. **确定性开关**。feature flag、灰度实验要能一键固定——跑分时这批东西还开着，分数里就混着实验噪音。
3. **稳定的关联键**。一次 prompt 引发的所有事件共享一个 prompt.id，tool_use_id 打通遥测和 hook 日志，message 级 ID 对齐落盘 transcript。Kimi 用响应头的 `x-trace-id` 串联，Codex 用 W3C traceparent 跨进程传播——三家都在埋关联 ID，因为这是 eval 和调试的刚需。
4. **结构化、可回放的 transcript**。三家的 JSONL 落盘格式（wire.jsonl、rollout、transcript）就是各自 eval 的事实数据源。

最后说一个反向发现。三家的公开材料里有 eval、有 CI、有 benchmark，但正文那个「门禁从软到硬的量化晋升门槛」——连续 5 次 PR 无 FAIL、误报 WARN 不超过 20% 才允许从告警升级为阻断——我在任何一家的公开材料里都没找到对应做法。这层治理目前是个空白，正文算是先填上了。
