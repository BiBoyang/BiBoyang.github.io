...
之所以这么看重这部分,是因为这部分关系到 Agent 能否成功开始运行,算得上最基础的步骤,这里做不好会直接指数型放大风险.而且我查阅资料后发现,专门讲述这部分的文章其实不算多,我就自己写.

倒不是说上下文和 Memory 不重要,而是这部分的问题往往更加隐蔽,而且讲述这方面的博文其实不少,各有优缺点,不好说那种观点一定正确.
...
这里想到什么就写什么,乱七八糟的,不过到这就结束了,以下的正文和后续,保证规规整整.
...
`ReAct` 这个名字来自 2022 年的一篇论文 **<<ReAct: Synergizing Reasoning and Acting in Language Models>>**（Yao et al., 2022），是 `Reasoning` 和 `Acting` 的合写。想法直白：**让大模型在"想"和"做"之间交替进行**。
...
注意看 `Thought` 那几行。模型在每一步都在"自言自语"——分析当前状态，决定下一步。这些思考过程用户平时看不到，但它们是 `ReAct` 的核心：正因为每步都在思考，模型才能根据实际看到的结果来调整策略。
...
**层级规划**。先定大方向，再逐层细化。比如大计划是"完成竞品分析"，子计划分别是"收集数据→分析对比→撰写报告"，每个子计划下面还有更细的步骤。复杂任务中，这种层级结构比一维的列表好管理。
