...
所以后面干脆拆成了几篇,当做我对Agent 推理模式的复盘。

之所以这么看重这部分,是因为这部分关系到 Agent 能否成功开始运行,这里没有灰色地带,执行失败就是失败.

而且倒不是说上下文和 memory 不重要,而是这部分的问题往往更加隐蔽,而且讲述这方面的博文其实不少,各有优缺点,不好说那种观点一定正确.

里面有些是我真的踩过的坑，有些是我顺着坑往下推时冒出来的担心，也有些是我翻资料、看源码时慢慢确认下来的东西。它不是一套很公整的教材，更像一份边踩边记、边查边想的工程笔记。中间肯定有我自己的猜测，也有不少胡思乱想，但这些猜测和胡思乱想，基本都尽量往具体系统行为、资料或者源码上靠进了。

为了文章不要过长,我删掉了一些实际遇到的问题和实践的操作,这部分内容以后再补充,这导致这系列文章实际上还是偏务虚.
