...
```
在这里，我们使用了 `strongSelf`。它的意义在于：一旦 `strongSelf` 成功建立，在这一小段 block 作用域里，对象就会被临时抓稳，不会在执行到一半的时候突然释放掉。

但是，这里依然存在一个微小的问题：       
* 我们知道，单独使用 `weakSelf` 的时候，是无法保证对象在整个 block 执行过程中都还活着的。虽然使用了 `strongSelf`，但是依然存在一种边界情况：`weakSelf` 在建立 `strongSelf` 之前就已经变成 `nil` 了。如果只是单纯地给 `self` 发送消息，其实问题不大，*OC 的消息机制保证了给 `nil` 发送消息不会直接崩溃*。

但是如果我们有其他操作，比如把 `self` 添加进数组，或者继续把它当成一个必须非空的对象来用，那这里就有可能出问题了。
...
而在 ARC 下要使用什么关键字呢？**`strong` 和 `copy` 都通常可以正常工作**。

通过之前几篇文章可以知道，在 ARC 中，编译器通常会在合适的时候把需要长期存在的 block 复制到堆上。也就是说，即使你写的是 `strong`，很多时候底层依然会出现 `copy` 语义上的堆迁移。所以从工程习惯上讲，写 `copy` 会更严谨一些，但写 `strong` 通常也不至于出问题。
