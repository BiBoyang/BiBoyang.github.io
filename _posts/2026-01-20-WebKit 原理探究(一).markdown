...
`URL Loading System` 会根据已注册的 `NSURLProtocol` 子类决定是否交由自定义协议处理.处理之后接着可以正常的转发请求给系统网络.
...
| WebProcess     | 渲染、JS 执行   | 多数情况每页 1 个 |
...
在这里,可以的得知,在 `WKWebView` 内部,是由一个 整体的 `Network Process` 来控制网络请求的,而传递数据则通过跨进程 `IPC` 进行传递.
