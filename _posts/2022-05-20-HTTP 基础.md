...
而基于 QUIC 的 HTTP/3 也已经完成标准化，可以查看 [RFC 9114](https://www.rfc-editor.org/rfc/rfc9114) 了解相关内容。


# 二. HTTP 基础

HTTP 是一个应用层协议，传统上主要使用 TCP 作为它的传输层基础。客户端会先和服务器建立 TCP 连接，连接建立之后再在其上发送 HTTP 报文。TCP 为 HTTP 提供可靠的数据传输服务，尽量保证请求和响应都能完整、有序地到达对端。

HTTP/1 本身并不为客户端维护协议层状态。假如某个特定用户在几秒钟内请求了几十次同一个对象，服务器不会因为“刚才已经处理过一次”就自动记住这件事，而是仍然把每个请求当作相对独立的消息来处理。所以我们说，HTTP 是**无状态协议**。当然，这并不意味着完全不能保存状态；后来为了补足这一点，才在 [RFC6265](https://tools.ietf.org/html/rfc6265) 中定义了 Cookie。
...
与很多常见回答不同，严格来讲，`GET` 和 `POST` 在“是否加密”这件事上并没有本质区别。如果说“`POST` 把参数放在 body 里所以就更安全”，那就有点太天真了 —— `HTTP` 本身就是一个**明文协议**！

更进一步地说，`GET` 和 `POST` 并不是“底层传输机制完全不同的两种东西”，它们本质上都只是 HTTP 方法，真正的区别主要体现在语义、缓存行为、幂等性以及服务端如何处理它们。
