...
```
我们发现了一个 `invoke` 方法。不过这里要强调一下：这更像是对 block 内部调用能力的一种运行时暴露，并不是我们日常业务开发里应该主动依赖的常规 API。
...
然后我们再调用 `void (*invoke)(void *,...)`。从原理展示的角度看，这里确实可以直接跳到 block 的函数指针去执行；但这种方式属于非常典型的“黑魔法”，更适合理解原理，不适合作为正常业务写法。
...
```
答：
这段代码最终会出问题。本质上是 `__block` 包装后的对象在执行过程中被置空，后续访问路径又没有做好保护，最后就很容易落到野指针 / 非法访问这类崩溃上。



# Question7 HookBlock
![](https://wx3.sinaimg.cn/mw690/51530583ly1fsatleo2zmj213u10caiu.jpg)
这一题就明显偏“原理展示”和“黑魔法”了。我才疏学浅，只对前两个有实现，第三个问题有思路但确实没写出来（😌）。
